    ...
    respond_to do |format|
      ...
      if user.present?
        if user.valid_password?(user_params[:password])
          # if user passed sign in successfully, create a token for him
          if user.active_for_authentication?
            user_token = user.user_tokens.create
            if user_token.persisted?
              format.json do
                # return user hash with access token inseide of it
            ...
            # user isn't active for any reason, eg. email not confirmed
            format.json do
